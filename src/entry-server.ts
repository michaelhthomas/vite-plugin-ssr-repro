...
export async function render(url: string, manifest) {
  const shouldPrerender = !url.includes('spa');
  ...
  if (shouldPrerender) {
    ...
    const newCtx = ctx as { modules: any }
    ...
    // request.
    const preloadLinks = renderPreloadLinks(newCtx.modules, manifest)
    return [html, preloadLinks, pageContext]
  } else {
    return ['', '', {}]
  }
}
